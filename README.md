...
3. **A single 72B true base run produced the clearest explicit structural-emergence lead so far, but its replication is now confounded.** Under neutral labels, `run_004_qwen25_72b_base.db` enacted `Grant Moderation to Role-A Agents` in the oligarchy and also passed several control-flavored title-only policies, including `Restrict Direct Messages`. A 10-seed follow-up batch (`run_006_qwen25_72b_base_batch10`) produced 0/10 repeats of the moderation grant, but this is *not* a clean falsification: commit `5383ad4` (between run_004 and the batch) removed the explicit menu of mechanical policy types from the `propose_policy` prompt. The pre/post prompt surfaces are confounded with the seed variance, so the original finding is best read as **unresolved** pending a clean prompt-controlled rerun. The directional democracy > oligarchy inequality pattern from run_004 did hold in 7/10 batch seeds, but at lower magnitudes than the original.
...
5. **Working hypothesis:** instruct / cooperative-assistant priors may wash out some institution-level behavior, causing instruct/RLHF-only evaluations to understate multi-agent risk. Immediate priority: resolve the prompt-surface confound for the 72B base condition (either re-run at pre-`5383ad4` or add a compiled-clause whitelist to the new prompt), then longer horizons, harsher scarcity, and larger populations with predeclared outcome criteria.
...
- **`N=1` for the curated model-comparison conditions, and the 72B-base replication attempt is prompt-confounded.** Each preserved 5-round model-condition pair has one run. A 10-seed base-model batch was run at the original configuration but after commit `5383ad4` changed the `propose_policy` prompt surface (removing the explicit menu of mechanical policy types). The batch did not reproduce `Grant Moderation to Role-A Agents` on any seed, but because the prompt surface also changed, seed variance and prompt-surface drift cannot be separated from that result. The 72B base power-consolidation finding is therefore neither replicated nor cleanly falsified.
